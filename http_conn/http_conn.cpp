...
void http_conn::init(){
    ...
    m_checked_index = 0;    //解析的字符
    m_start_line = 0;   //解析的行的索引
    m_read_idx = 0; //标识读缓冲区已经读入的客户端数据的最后一个字节的下一个位置
    m_write_idx = 0;
    m_method = GET;
    m_url = 0;
    m_version = 0;
...
bool http_conn::write(){
    ...
    while(1) {
        // 分散写，将多块不连续内存的内容一起写出去
        //参数为写到哪，和iovec的结构体存储数据起始地址，和有几块起始地址
...
http_conn::HTTP_CODE http_conn::parse_request_line(char * text){
    ...
    *m_version++ = '\0';
    ...
    if(strncasecmp(m_url, "http://", 7) == 0){
        m_url+=7;
        //strpbrk支持多个字符搜索，这里不需要，故使用strchr
        m_url = strchr(m_url, '/');
    }
    ...
    //因为只读取了请求头，还没有检查完毕，返回NO_REQUEST
    return NO_REQUEST;
}
...
bool http_conn::add_response( const char* format, ... ) {
    ...
    //vsnprintf将格式化的参数输出到串中，返回合并后的长度，参数为数组首地址，数组大小，和需要拼的参数，即将format内容写入到m_write_buf中
...
void http_conn::process(){
    ...
    if(read_ret == NO_REQUEST){
        //需要继续读取http请求，继续监听m_socked文件描述符
        //(因为注册了oneshot事件，触发了一次之后就将事件从epollfd上清除了，需要用EPOLL_CTL_COD来手动加上EPOLLIN事件)
        modfd(m_epollfd, m_sockfd, EPOLLIN);
        return;
    }
    //生成响应
